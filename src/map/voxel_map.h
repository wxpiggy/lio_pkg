...
namespace std {
template <>
struct hash<wxpiggy::VOXEL_LOCATION> {
    int64_t operator()(const wxpiggy::VOXEL_LOCATION &s) const {
        using std::hash;
        using std::size_t;
...
struct State{
    ...
    Eigen::Vector3d trans_ = Eigen::Vector3d::Zero();
...
class VoxelMapManager {
   ...
   private:
    ...
    //   void pubSinglePlane(visualization_msgs::MarkerArray &plane_pub, const std::string plane_ns, const VoxelPlane
    //   &single_plane, const float alpha,
