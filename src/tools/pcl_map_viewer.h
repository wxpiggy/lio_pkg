...
// template class pcl::visualization::PointCloudGeometryHandlerXYZ<wxpiggy::Point>;
namespace wxpiggy{
...
class PCLMapViewer {
   public:
    ...
    void SaveMap(std::string path) {
        if (local_map_->size() > 0) {
            wxpiggy::SaveCloudToFile(path, *local_map_);
   ...
   private:
    pcl::VoxelGrid<wxpiggy::PointType> voxel_filter_;
