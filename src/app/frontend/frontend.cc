// #include "frontend.h"
// #include "slam/frontend/lio_iekf.h"
// #include "slam/frontend//loosely_lio.h"
// #include "../ros_publisher.h"
// // #include "common/io_utils.h"

// #include <yaml-cpp/yaml.h>

// namespace wxpiggy {

// Frontend::Frontend(const std::string& config_yaml) { config_yaml_ = config_yaml; }

// bool Frontend::Init(ros::NodeHandle& nh) {
//     LOG(INFO) << "load yaml from " << config_yaml_;
//     auto yaml = YAML::LoadFile(config_yaml_);
//     try {
//         auto n = yaml["bag_path"];
//         LOG(INFO) << Dump(n);
//         bag_path_ = yaml["bag_path"].as<std::string>();
//         lio_yaml_ = yaml["lio_yaml"].as<std::string>();
//     } catch (...) {
//         LOG(ERROR) << "failed to parse yaml";
//         return false;
//     }

//     system("rm -rf ./data/ch9/*.pcd");
//     system("rm -rf ./data/ch9/keyframes.txt");
//     LioIEKF* lio = new  wxpiggy::LioIEKF();
//      wxpiggy::ROSPublisher ros_publisher(nh);
//     lio ->setFunc(ros_publisher.GetCloudPublishFunc());
//     lio ->setFunc(ros_publisher.GetPosePublishFunc());
//     lio->Init(config_yaml_);

//     return true;
// }

// void Frontend::Run() {
// //     sad::RosbagIO rosbag_io(bag_path_, DatasetType::NCLT);

// //     // 先提取RTK pose，注意NCLT只有平移部分
// //     rosbag_io
// //         .AddAutoRTKHandle([this](GNSSPtr gnss) {
// //             gnss_.emplace(gnss->unix_time_, gnss);
// //             return true;
// //         })
// //         .Go();
// //     rosbag_io.CleanProcessFunc();  // 不再需要处理RTK

//     RemoveMapOrigin();

//     // 再运行LIO
//     // rosbag_io
//     //     .AddAutoPointCloudHandle([&](sensor_msgs::PointCloud2::Ptr cloud) -> bool {
//     //         lio_->PCLCallBack(cloud);
//     //         ExtractKeyFrame(lio_->GetCurrentState());
//     //         return true;
//     //     })
//     //     .AddImuHandle([&](IMUPtr imu) {
//     //         lio_->IMUCallBack(imu);
//     //         return true;
//     //     })
//     //     .Go();
//     lio_->Finish();

//     // 保存运行结果
//     SaveKeyframes();

//     LOG(INFO) << "done.";
// }

// void Frontend::ExtractKeyFrame(const wxpiggy::NavStated& state) {
//     if (last_kf_ == nullptr) {
//         if (!lio_->GetCurrentScan()) {
//             // LIO没完成初始化
//             return;
//         }
//         // 第一个帧
//         auto kf = std::make_shared<Keyframe>(state.timestamp_, kf_id_++, state.GetSE3(), lio_->GetCurrentScan());
//         FindGPSPose(kf);
//         kf->SaveAndUnloadScan("./data/ch9/");
//         keyframes_.emplace(kf->id_, kf);
//         last_kf_ = kf;
//     } else {
//         // 计算当前state与kf之间的相对运动阈值
//         SE3 delta = last_kf_->lidar_pose_.inverse() * state.GetSE3();
//         if (delta.translation().norm() > kf_dis_th_ || delta.so3().log().norm() > kf_ang_th_deg_ * math::kDEG2RAD) {
//             auto kf = std::make_shared<Keyframe>(state.timestamp_, kf_id_++, state.GetSE3(), lio_->GetCurrentScan());
//             FindGPSPose(kf);
//             keyframes_.emplace(kf->id_, kf);
//             kf->SaveAndUnloadScan("./data/ch9/");
//             LOG(INFO) << "生成关键帧" << kf->id_;
//             last_kf_ = kf;
//         }
//     }
// }

// void Frontend::FindGPSPose(std::shared_ptr<Keyframe> kf) {
//     SE3 pose;
//     GNSSPtr match;
//     if (math::PoseInterp<GNSSPtr>(
//             kf->timestamp_, gnss_, [](const GNSSPtr& gnss) -> SE3 { return gnss->utm_pose_; }, pose, match)) {
//         kf->rtk_pose_ = pose;
//         kf->rtk_valid_ = true;
//     } else {
//         kf->rtk_valid_ = false;
//     }
// }

// void Frontend::SaveKeyframes() {
//     std::ofstream fout("./data/ch9/keyframes.txt");
//     for (auto& kfp : keyframes_) {
//         kfp.second->Save(fout);
//     }
//     fout.close();
// }

// void Frontend::RemoveMapOrigin() {
//     if (gnss_.empty()) {
//         return;
//     }

//     bool origin_set = false;
//     for (auto& p : gnss_) {
//         if (p.second->status_ == GpsStatusType::GNSS_FIXED_SOLUTION) {
//             map_origin_ = p.second->utm_pose_.translation();
//             origin_set = true;

//             LOG(INFO) << "map origin is set to " << map_origin_.transpose();

//             auto yaml = YAML::LoadFile(config_yaml_);
//             std::vector<double> ori{map_origin_[0], map_origin_[1], map_origin_[2]};
//             yaml["origin"] = ori;

//             std::ofstream fout(config_yaml_);
//             fout << yaml;
//             break;
//         }
//     }

//     if (origin_set) {
//         LOG(INFO) << "removing origin from rtk";
//         for (auto& p : gnss_) {
//             p.second->utm_pose_.translation() -= map_origin_;
//         }
//     }
// }

// }  // namespace sad
#include "frontend.h"
#include "slam/frontend/lio_iekf.h"
#include "slam/frontend/loosely_lio.h"
#include "../ros_publisher.h"
#include "common/gnss.h"
#include "tools/utm_convert.h"
#include <yaml-cpp/yaml.h>
#include <sensor_msgs/PointCloud2.h>
#include <sensor_msgs/Imu.h>

namespace wxpiggy {

Frontend::Frontend(const std::string& config_yaml) { 
    config_yaml_ = config_yaml; 
    map_origin_set_ = false;
}

bool Frontend::Init(ros::NodeHandle& nh) {
    LOG(INFO) << "load yaml from " << config_yaml_;
    auto yaml = YAML::LoadFile(config_yaml_);
    // try {
    //     lio_yaml_ = yaml["lio_yaml"].as<std::string>();
        
    //     // 尝试从配置文件中读取已有的原点
    //     // if (yaml["origin"]) {
    //     //     auto origin = yaml["origin"].as<std::vector<double>>();
    //     //     if (origin.size() == 3) {
    //     //         map_origin_ = Eigen::Vector3d(origin[0], origin[1], origin[2]);
    //     //         map_origin_set_ = true;
    //     //         LOG(INFO) << "Loaded map origin from config: " << map_origin_.transpose();
    //     //     }
    //     // }
    // } catch (...) {
    //     LOG(ERROR) << "failed to parse yaml";
    //     return false;
    // }

    // 清理数据目录
    system("rm -rf /catkin_ws/src/lio_pkg/data/*.pcd");
    system("rm -rf /catkin_ws/src/lio_pkg/data/keyframes.txt");
    
    // 初始化LIO
    lio_ = std::make_shared<LioIEKF>();

    ros_publisher_ = std::make_shared<ROSPublisher>(nh);
    
    lio_->setFunc(ros_publisher_->GetCloudPublishFunc());
    lio_->setFunc(ros_publisher_->GetPosePublishFunc());
    lio_->Init(config_yaml_);

    // 创建ROS订阅器
    subPointCloud_ = nh.subscribe<sensor_msgs::PointCloud2>(
        "/points_raw", 10, &Frontend::PointCloudCallback, this);
    
    subImu_ = nh.subscribe<sensor_msgs::Imu>(
        "/imu_raw", 100, &Frontend::ImuCallback, this);
    
    // GNSS订阅
    subGnss_ = nh.subscribe<sensor_msgs::NavSatFix>("gps_rtk_fix", 10, &Frontend::GnssCallback, this);

    return true;
}

void Frontend::GnssCallback(const sensor_msgs::NavSatFix::ConstPtr& gnss_msg){
    if (gnss_msg == nullptr) {
        return;
    }
    
    GNSSPtr gnss(new GNSS(gnss_msg));
    ConvertGps2UTMOnlyTrans(*gnss);
    
    if (std::isnan(gnss->lat_lon_alt_[2])) {
        return;
    }
    
    // 在线设置地图原点
    if (!map_origin_set_ && gnss->status_ == GpsStatusType::GNSS_FIXED_SOLUTION) {
        SetMapOriginOnline(gnss);
    }
    
    // 如果原点已设置，移除原点偏移
    if (map_origin_set_) {
        gnss->utm_pose_.translation() -= map_origin_;
    }
    
    gnss_.emplace(gnss->unix_time_, gnss);
    LOG(INFO) << "gnss callbakc";
}

void Frontend::SetMapOriginOnline(GNSSPtr gnss) {
    std::lock_guard<std::mutex> lock(origin_mutex_);
    
    if (!map_origin_set_) {
        map_origin_ = gnss->utm_pose_.translation();
        map_origin_set_ = true;
        
        LOG(INFO) << "Online map origin set to: " << map_origin_.transpose();
        
        // 保存原点到配置文件
        SaveMapOriginToConfig();
        
        // 对之前接收到的所有GNSS数据应用原点偏移
        ApplyOriginToHistoricalGNSS();
    }
}

void Frontend::SaveMapOriginToConfig() {
    try {
        auto yaml = YAML::LoadFile(config_yaml_);
        std::vector<double> origin{map_origin_[0], map_origin_[1], map_origin_[2]};
        yaml["origin"] = origin;
        
        std::ofstream fout(config_yaml_);
        fout << yaml;
        LOG(INFO) << "Map origin saved to config file";
    } catch (...) {
        LOG(ERROR) << "Failed to save map origin to config file";
    }
}

void Frontend::ApplyOriginToHistoricalGNSS() {
    std::lock_guard<std::mutex> lock(gnss_mutex_);
    for (auto& pair : gnss_) {
        pair.second->utm_pose_.translation() -= map_origin_;
    }
    LOG(INFO) << "Applied origin offset to " << gnss_.size() << " historical GNSS points";
}

void Frontend::PointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    lio_->PCLCallBack(cloud_msg);
    ExtractKeyFrame(lio_->GetCurrentState());
}

void Frontend::ImuCallback(const sensor_msgs::Imu::ConstPtr& imu_msg) {
    IMUPtr imu = std::make_shared<IMU>();
    imu->timestamp_ = imu_msg->header.stamp.toSec();
    imu->gyro_ = Eigen::Vector3d(
        imu_msg->angular_velocity.x,
        imu_msg->angular_velocity.y, 
        imu_msg->angular_velocity.z
    );
    imu->acce_ = Eigen::Vector3d(
        imu_msg->linear_acceleration.x,
        imu_msg->linear_acceleration.y,
        imu_msg->linear_acceleration.z
    );
    
    lio_->IMUCallBack(imu);
}

void Frontend::Run() {
    LOG(INFO) << "Frontend started, waiting for GNSS fixed solution to set map origin...";
    ros::spin();
    
    lio_->Finish();
    SaveKeyframes();
    LOG(INFO) << "Frontend shutdown complete";
}

void Frontend::ExtractKeyFrame(const wxpiggy::NavStated& state) {
    if (last_kf_ == nullptr) {
        if (!lio_->GetCurrentScan()) {
            return;
        }
        auto kf = std::make_shared<Keyframe>(state.timestamp_, kf_id_++, state.GetSE3(), lio_->GetCurrentScan());
        FindGPSPose(kf);
        kf->SaveAndUnloadScan("/catkin_ws/src/lio_pkg/data/");
        keyframes_.emplace(kf->id_, kf);
        last_kf_ = kf;
        
        // 记录第一个关键帧的信息
        LOG(INFO) << "First keyframe created at time: " << state.timestamp_;
    } else {
        SE3 delta = last_kf_->lidar_pose_.inverse() * state.GetSE3();
        if (delta.translation().norm() > kf_dis_th_ || delta.so3().log().norm() > kf_ang_th_deg_ * math::kDEG2RAD) {
            auto kf = std::make_shared<Keyframe>(state.timestamp_, kf_id_++, state.GetSE3(), lio_->GetCurrentScan());
            FindGPSPose(kf);
            keyframes_.emplace(kf->id_, kf);
            kf->SaveAndUnloadScan("/catkin_ws/src/lio_pkg/data/");
            LOG(INFO) << "生成关键帧" << kf->id_ << ", total: " << keyframes_.size();
            last_kf_ = kf;
        }
    }
}

void Frontend::FindGPSPose(std::shared_ptr<Keyframe> kf) {
    std::lock_guard<std::mutex> lock(gnss_mutex_);
    SE3 pose;
    GNSSPtr match;
    if (math::PoseInterp<GNSSPtr>(
            kf->timestamp_, gnss_, [](const GNSSPtr& gnss) -> SE3 { return gnss->utm_pose_; }, pose, match)) {
        kf->rtk_pose_ = pose;
        kf->rtk_valid_ = true;
        if (map_origin_set_) {
            LOG(INFO) << "Keyframe " << kf->id_ << " has valid GPS pose (origin applied)";
        }
    } else {
        kf->rtk_valid_ = false;
        LOG(WARNING) << "Keyframe " << kf->id_ << " no GPS pose found";
    }
}

void Frontend::SaveKeyframes() {
    std::ofstream fout("/catkin_ws/src/lio_pkg/data/keyframes.txt");
    for (auto& kfp : keyframes_) {
        kfp.second->Save(fout);
    }
    fout.close();
    LOG(INFO) << "Saved " << keyframes_.size() << " keyframes";
}


}  // namespace wxpiggy