...
VoxelLioIEKF::VoxelLioIEKF(Options options) : options_(options) {
    VoxelMapConfig config;
    voxel_map_ptr.reset(new VoxelMapManager(config, voxel_map));
...
bool VoxelLioIEKF::Init(const std::string &config_yaml) {
    if (!LoadFromYAML(config_yaml)) {
        LOG(INFO) << "init failed.";
        return false;
    }
    voxel_map_ptr->setExt(TIL_);
...
void VoxelLioIEKF::ProcessMeasurements(const MeasureGroup &meas) {
...
bool VoxelLioIEKF::LoadFromYAML(const std::string &yaml_file) {
...
void VoxelLioIEKF::Align() {
    ...
    CloudPtr world_scan(new PointCloudType);
    pcl::transformPointCloud(*current_scan_filter, *world_scan, TIL_.matrix().cast<float>());
    /// the first scan
    if (flg_first_scan_) {
        voxel_map_ptr->setInputCloud(current_scan_filter, world_scan);
        voxel_map_ptr->BuildVoxelMap();
    ...
    voxel_map_ptr->setInputCloud(current_scan_filter);
    ieskf_.UpdateUsingCustomObserveWithCov(
        [this](const SE3 &input_pose, const Eigen::Matrix<double, 6, 6> &cov, Mat18d &HTVH, Vec18d &HTVr) {
            voxel_map_ptr->setState(input_pose, cov);
            voxel_map_ptr->buildResidualandJacobians(HTVH, HTVr);
    ...
    auto cov = ieskf_.getCov();
    SE3 delta_pose = last_pose_.inverse() * current_pose;
    pcl::PointCloud<pcl::PointXYZI>::Ptr world_lidar(new pcl::PointCloud<pcl::PointXYZI>());
    voxel_map_ptr->pv_list_.clear();
    voxel_map_ptr->pv_list_.resize(current_scan_filter->size());
    voxel_map_ptr->TransformLidar(
        current_pose.rotationMatrix(), current_pose.translation(), current_scan_filter, world_lidar);
    for (size_t i = 0; i < world_lidar->points.size(); i++) {
        voxel_map_ptr->pv_list_[i].point_w << world_lidar->points[i].x, world_lidar->points[i].y,
            world_lidar->points[i].z;
        Eigen::Matrix3d point_crossmat = voxel_map_ptr->cross_mat_list_[i];
        Eigen::Matrix3d var = voxel_map_ptr->body_cov_list_[i];
        var = (current_pose.rotationMatrix() * TIL_.rotationMatrix()) * var *
                  (current_pose.rotationMatrix() * TIL_.rotationMatrix()).transpose() +
              (-point_crossmat) * cov.block<3, 3>(0, 0) * (-point_crossmat).transpose() + cov.block<3, 3>(3, 3);
        voxel_map_ptr->pv_list_[i].var = var;
    }
    // voxel_map_ptr->UpdateVoxelMap(voxel_map_ptr->pv_list_);
...
void VoxelLioIEKF::TryInitIMU() {
    ...
    if (imu_init_.InitSuccess()) {
        // 读取初始零偏，设置ESKF
        wxpiggy::IESKFD::Options options;
        // 噪声由初始化器估计
...
void VoxelLioIEKF::PCLCallBack(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    sync_->ProcessCloud(msg);
}

void VoxelLioIEKF::LivoxPCLCallBack(const livox_ros_driver::CustomMsg::ConstPtr &msg) {
    sync_->ProcessCloud(msg);
}

void VoxelLioIEKF::IMUCallBack(IMUPtr msg_in) {
    sync_->ProcessIMU(msg_in);
}

void VoxelLioIEKF::Finish() {
    if (options_.with_ui_) {
